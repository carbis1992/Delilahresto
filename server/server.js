...
// DB setup/connection
const { conf_db_host, conf_db_name, conf_user, conf_password, conf_port } = require("../database/connection-db");
...
server.get("/users/login", async (req, res, next) => {
...
server.get("/users", validate_token, async (req, res, next) => {
...
server.put("/users", validate_token, async (req, res, next) => {
...
server.delete("/users", validate_token, async (req, res, next) => {
	const token = req.token_info;
...
server.get("/users/:username", validate_token, is_admin, async (req, res, next) => {
...
server.put("/users/:username", validate_token, is_admin, async (req, res, next) => {
	const username = req.params.username;
	try {
		const found_user = await get_by_param("users", "username", username);
		const user_id = found_user.user_id;
...
server.delete("/users/:username", validate_token, is_admin, async (req, res, next) => {
	const username = req.params.username;
	try {
		const found_user = await get_by_param("users", "username", username);
		const user_id = found_user.user_id;
...
// PRODUCTS
server.get("/products", validate_token, async (req, res, next) => {
	const products = await get_by_param("products", "is_disabled", false, true);
	res.status(200).json(products);
});
server.post("/products", validate_token, is_admin, async (req, res, next) => {
...
server.get("/products/:id", validate_token, async (req, res, next) => {
...
server.put("/products/:id", validate_token, is_admin, async (req, res, next) => {
	const product_id = req.params.id;
	try {
		const product_found = await get_by_param("products", "product_id", product_id);
		if (product_found) {
			...
			const update = await sequelize.query(
				"UPDATE products SET name = :name, price = :price, is_disabled = :is_disabled WHERE product_id = :product_id",
				{
					replacements: {
...
server.delete("/products/:id", validate_token, is_admin, async (req, res, next) => {
	const product_id = req.params.id;
	try {
		const product_found = await get_by_param("products", "product_id", product_id);
		if (product_found) {
...
// Orders
server.get("/orders", validate_token, async (req, res, next) => {
	try {
...
server.post("/orders", validate_token, async (req, res, next) => {
	const user_id = req.token_info.user_id;
...
server.get("/orders/:id", validate_token, is_admin, async (req, res, next) => {
...
server.put("/orders/:id", validate_token, is_admin, async (req, res, next) => {
	const order_id = req.params.id;
...
server.delete("/orders/:id", validate_token, is_admin, async (req, res, next) => {
	const order_id = req.params.id;
	try {
...
// Test Endpoints
server.get("/validate-token", validate_token, async (req, res, next) => {
